...
[![fern shield](https://img.shields.io/badge/%F0%9F%8C%BF-Built%20with%20Fern-brightgreen)](https://buildwithfern.com?utm_source=github&utm_medium=github&utm_campaign=readme&utm_source=Sandbox%2FGo)
...
```go
response, err := client.Shell.ExecCommand(...)
if err != nil {
...
```go
response, err := client.Shell.WithRawResponse.ExecCommand(...)
if err != nil {
    return err
}
...
response, err := client.Shell.ExecCommand(ctx, ...)
```
...
response, err := client.Shell.ExecCommand(ctx, request, ...)
```
